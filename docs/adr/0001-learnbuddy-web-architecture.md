...
- **Test strategy**: pure logic (segmentation, language detection, history, rate mapping, loop state machines, error mapping) with `node --test` on the dev machine; backend logic with Python `unittest` (no network; Japanese epub parsing uses SudachiPy — ADR 0007); browser behavior on real devices belongs to 「家庭设备真机验收」 (issue #6). zh/ja/en segmentation parity cases lock Intl.Segmenter vs ICU4J expectations.
