...
**已由原型验证**（一次性分支 `prototype/narrow-ia`，票 #41）。原型做了三个结构上不同的窄屏导航（**汉堡+抽屉** / **底部标签+抽屉** / **无抽屉**），**三者都成立**；下面的形态与六条实现要求全部由实测得出。

| 宽屏元素 | 窄屏形态 |
|---|---|
| 左导航 | **抽屉**（点汉堡或从左缘划入）—— 里面仍是同一个侧栏。**底部标签是可接受的等价**（后两个变体） |
| D3 | **全屏覆盖层**（从右滑入、占满、带返回） |
| 正文 | **单栏全宽**（左右留 padding） |
| 词卡 | **底部卡片**（`max-height 62%`），不再贴着词 |
| 选中工具条 | 仍浮在选区上方 |
| 顶栏 | 根位置是 `☰`（或头像）；**`☰` 与 `‹` 不得共存** —— 覆盖层自带 `‹` |

**六条实现要求（全部实测得来，不是风格偏好）：**

1. **层叠是结构性的，不需要额外规则。** D3 是全屏层，它出现时抽屉必须已收起（实测：抽屉开着时页面上**没有任何可达的 D3 触发点**）。返回逐层关闭：D3 → 词卡 → 身份 → 想法 → 工具条 → 抽屉。
2. **打开/关闭 D3 不得重建阅读面。** 实测第一版把阅读位置弄丢了（`1049 → 0 → 0`）：整体重建 `#main` 会重置 `.pane` 的 `scrollTop`。**「D3 是覆盖层所以阅读面还在」只在你不重渲染时成立。**
3. **滚动位置必须实时接住，不许缓存。** 修好第 2 条后又踩第二个坑：一个被 `resize` 写过的缓存偏移量**永久遮蔽**了实时值（实测 `keep=1049` 却用了 `0`），把视图钉在顶部。⇒ 加一条验收：**开关 D3 后滚动位置不变。**
4. **开词卡时必须把被查的词滚进卡片上方的可视区**（留 ~24px），否则卡片会遮住它要解释的那个词。
5. **底部输入框要复用现役应用的虚拟键盘处理。** 原型的截图复现了 **iOS Safari 的真实故障模式**：键盘覆盖布局视口而不缩小它 ⇒ 底部 sheet 的输入框被盖住。`web/css/style.css` 里**已有** `visualViewport` 处理（CONTEXT.md 记着「触屏聚焦时接管虚拟键盘上方的全部空间」）—— **新壳的底部 sheet 不能另起一套。**
6. **选中工具条在键盘弹起时收起**（实测 `selbar:false`），输入交给底部 sheet。

**断点：`~900px` 够用。** 三栏需要 238（导航）+ 正文 + 378（D3）；834px 下只剩 **218px 给正文**，不可读 ⇒ **平板竖屏（768–834）归窄屏是对的**。
...
- [ ] **窄屏**：抽屉/底部标签 + D3 全屏覆盖层 + 单栏 + 底部词卡（**原型已验**，见 §4.2）；
- [ ] **开关 D3 后阅读位置不变**（§4.2 第 2、3 条的两个坑都踩过）
- [ ] **底部输入框不被虚拟键盘盖住**（复用现役 `visualViewport` 处理）
...
**原型（一次性分支）**：`prototype/shell-ia`（宽屏，`r3.html` 是赢家，默认 C2）· `prototype/narrow-ia`（窄屏，三变体 + 五问答案）
