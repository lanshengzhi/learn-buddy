...
- **Word state** — A **Profile**'s mark on a word from a **Lookup**: 我认识 or unmarked, two states only. Stored per Profile and shared across all **Books**, keyed by the resolved **Word key** (`ja:食べる`, `en:run`, `zh:宝玉`), so inflected forms share one state; words marked 我认识 are the ones the 生词提示 machinery leaves alone.
