...
Learner records live **only on the server**, keyed by **Person**: reading position, History, word states and playback preferences. The browser keeps one key (`lb.profile` — who is using this device) plus the Service Worker shell cache; no learner state and no audio. (ADR 0007)
...
- **Rate preset** — One of six learner-selectable speech rates — 0.5×, 0.75×, 1×, 1.25×, 1.5×, 2× — mapped linearly to SSML rates (`-50%`, `-25%`, `+0%`, `+25%`, `+50%`, `+100%`; 2× is the upstream +100% ceiling). One preset per **Person** applies to every passage and language and persists across sessions; switching Person switches the preset. Selecting a preset re-synthesizes the current sentence at that rate.
...
- **Server audio cache** — The backend's cache of MP3 bytes keyed by the SHA-256 hash of `text|voice|rate` (for Japanese, the **Reading-normalized** text plus the normalization version), shared by all family devices and Persons. It is the **only** audio cache: the browser stores no audio (ADR 0007), so replaying a sentence needs the LAN.
...
- **API error** — The same `{"error": code}` shape for the Book and Person endpoints: `bad_request`, `profile_not_found`, `book_not_found`, `chapter_not_found`, `entry_not_found`, `not_found`, `too_large`, `not_epub`, `parse_failed`, and for the lookup / AI layer `lookup_unavailable`, `ai_not_configured`, `ai_upstream_error`, `ai_timeout` (plus `network_failure` and `unknown`; ADR 0008).

## Learners and records

- **Person (人)** — One member of the family: *who this is*. **Renamed from Profile** (ADR 0011) — the entity outgrew "who is reading" and now owns a **Conversation** and **Memory** as well as reading records. **Ownership and reachability are separate statements**: each Person's reading positions, History, word states, preferences, Conversations and Memory are theirs, but there is no password, no login and no data isolation — anyone may switch to anyone. The list is a read-only `profiles.json` on the server and the device remembers the last choice in `lb.profile`; those identifiers still spell the old word until the API is rebuilt (ADR 0011).
- **Word state** — A **Person**'s mark on a word from a **Lookup**: 我认识 or unmarked, two states only. Stored per Person and shared across all **Books**, keyed by the resolved **Word key** (`ja:食べる`, `en:run`, `zh:宝玉`), so inflected forms share one state; words marked 我认识 are the ones the 生词提示 machinery leaves alone.
- **Highlight (划线)** — A **Person**'s mark on one **Sentence**, drawn as a background colour. Sentence-level on purpose: a Sentence is the unit the whole app already speaks in (playback, **Reading position**, lookup), so a Highlight reuses `Reading position`'s anchor — chapter plus sentence index, with the sentence's opening text for re-anchoring after a re-parse. Words are never marked this way; that is **Word state** and 生词提示. A Highlight is **Reference**-able: `@` a **Chapter**'s Highlights hands them to a **Conversation**. (ADR 0011)
- **Note (想法)** — A **Person**'s plain-text remark on one **Sentence** — one Note per Sentence, shown under it, same anchor as a **Highlight**. (ADR 0011)
- **Word key** — The storage identity of a looked-up word: the language prefix plus the **dictionary form** the lookup resolved to (`ja:食べる`, `en:run`, `zh:宝玉`). Inflected surfaces (食べられ, running) resolve to the same key, so a mark made on one form applies to all of them; in Chinese the key is the simplified form, so a traditional hit (寶玉) shares the mark with its simplified spelling.
...
- **History** — A per-**Person**, learner-visible list of texts submitted from the **Editor**, stored on the server. It survives sessions and devices, and is bounded to 50 entries. Books are never History entries.
...
- **Delete history entry** — The learner-initiated removal of a History entry from the History list. The entry disappears immediately, for that Person on every device.
...
- **Book** — One uploaded epub in the **Library**, with its own **Chapter** structure and a **Reading position** per **Person**. The app opens one Book at a time; reopening the app returns to the last one opened by the active Person.
- **Chapter** — A section of a **Book**, taken from the epub's own navigation; the unit of reading, playback, and progress.
- **Reading position** — Where in a **Book** a **Person** last stopped, stored on the server as a chapter plus sentence index (with the sentence's opening text for re-anchoring after a re-parse), so it follows that Person across devices; the app returns there when the Book is opened again.
- **Lookup (查义)** — An on-demand query on a character or word for its reading and meaning, served by the server over build-time SQLite dictionaries (`en`/`ja`/`zh` + kanji and rare-character fallbacks under `<data-dir>/dicts/`; ADR 0008, 0009). The card first shows the local dictionary entry (读音 + senses); the AI context explanation is an optional layer *on top of* it (separate tab), never part of the Lookup itself. A word missing from the dictionary is a card state (「词典里没有这个词。」), never a hidden failure.
- **生词提示 (word highlight)** — The Reading surface's optional vocabulary cue over a **Chapter**'s word spans, in three modes: 标生词 (default; a soft underline on words present in the dictionary and not yet marked 我认识), 淡已认识 (known words dimmed), and 关. Presence is resolved by lazy batch checks for visible sentences (`POST /lookup/check`), never by baking at upload time; the mode is a per-Person preference (`/state`'s `hl_mode`).
- **Dictionary form resolution** — The server's lookup chain (ADR 0008, 0009): English resolves by lowercase exact match with mechanical lemma fallback (suffix strips); Japanese walks surface → normalized → dictionary form → reading (Sudachi) with a single-kanji KANJIDIC2 fallback; Chinese exact-matches the surface against CC-CEDICT's two traditions (the resolved key is the simplified form) and falls back to Unihan for a single rare character; other languages miss for now.
...
| `account`, `login`, `user` | `Person` | No authentication exists: a Person is only *who this is*; passwords and permissions are out of scope. |
| `agent`, assistant, bot | — | The product has no such entity: a Person has **Conversations** and **Memory**. "Agent" is runtime vocabulary (pi / DSH), not product language. (ADR 0011) |
